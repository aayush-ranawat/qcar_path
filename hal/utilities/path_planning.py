...
class RoadMap:
    ...
    def set_node_pose(self, nodeID, x, y, theta):
        """Set the pose of a node and recalculate all connected edges.
        ...
        node.pose = np.array([x, y, theta]).reshape(3, 1)
        for edge in node.outEdges + node.inEdges:
            self._calculate_trajectory(edge, edge.radius)
    ...
    def generate_path(self, nodeSequence):
        ...
        for i in range(len(nodeSequence) - 1):
            a, b = nodeSequence[i], nodeSequence[i+1]
            pathSegment = self.find_shortest_path(a, b)
            if pathSegment is None:
                raise ValueError(
                    f"No path found between node {a} and node {b} "
                    f"(indices {i} and {i+1} in nodeSequence). "
                    f"Check that a directed edge chain exists between them."
                )
